...
const COMPONENT_COLORS: Record<CreditCardComponentType, string> = {
...
function bucketComponent(raw: string | null): CreditCardComponentType | 'Other' {
  if (!raw) return 'Other';
...
function increasesBalance(c: CreditCardComponentType | 'Other'): boolean {
  return c !== 'Other' && POSITIVE_TO_BALANCE.includes(c);
}
...
export default function CreditCardDetailPage() {
  ...
  const [cardQuery, txQuery, categoriesQuery] = queries;
  ...
  const totals = useMemo(() => {
    const acc: Record<CreditCardComponentType | 'Other' | 'Total' | 'Net', number> = {
      Charge: 0,
      Payment: 0,
      Interest: 0,
      Fee: 0,
    ...
    for (const t of transactions) {
      ...
      acc[bucket] += t.amount;
      acc.Total += t.amount;
      if (increasesBalance(bucket)) acc.Net += t.amount;
  ...
  const monthlyChartData = useMemo(() => {
    ...
    for (const t of transactions) {
      ...
      const signed = increasesBalance(bucket) ? t.amount : -t.amount;
  ...
  const balanceTrajectory = useMemo(() => {
    ...
    for (const t of sortedDesc) {
      ...
      const bucket = bucketComponent(t.componentType);
      const signedDelta = increasesBalance(bucket) ? t.amount : -t.amount;
  ...
  const yearOverYear = useMemo<YearTotals[]>(() => {
    ...
    for (const t of transactions) {
      ...
      if (bucket !== 'Other') {
        row[bucket] += t.amount;
        row.Total += t.amount;
        if (increasesBalance(bucket)) row.Net += t.amount;
  ...
  const openEdit = (t: CreditCardTransaction) => {
    ...
    setComponentType(ct === 'Other' ? 'Charge' : ct);
        ...
        {transactions.length === 0 ? (
          ...
          <Table hover responsive size="sm" className="mb-0">
            ...
            <tbody>
              {transactions.map((t) => {
                const ct = bucketComponent(t.componentType);
                const debtUp = increasesBalance(ct);
                const color = ct === 'Other' ? undefined : COMPONENT_COLORS[ct as CreditCardComponentType];
                return (
